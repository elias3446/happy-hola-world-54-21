...
function CenterOnUserButton({ userPosition }: { userPosition: [number, number] }) {
  ...
  const handleCenterClick = (e: React.MouseEvent) => {
    try {
      // Prevent any event bubbling that might cause navigation
      e.preventDefault();
      e.stopPropagation();
      
      console.log('Center button clicked, user position:', userPosition);
      
      if (userPosition[0] !== 0 && userPosition[1] !== 0) {
        // Use flyTo with error handling
...
const MapaBase: React.FC<MapaBaseProps> = ({
  ...
  return (
    <div className={cn(
      ...
      {/* Search bar positioned at the top with responsive width */}
      {!hideSearchBar && (
        <div className="absolute top-4 left-1/2 transform -translate-x-1/2 z-[1000] w-full max-w-[200px] sm:max-w-xs md:max-w-sm lg:max-w-md px-2 sm:px-4">
          <SearchBar 
