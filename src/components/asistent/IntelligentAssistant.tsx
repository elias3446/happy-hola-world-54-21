...
const IntelligentAssistant: React.FC = () => {
  ...
  return (
    <div className="flex flex-col w-full min-h-screen bg-background overflow-hidden">
      {/* Header responsive */}
      <div className="bg-gradient-to-r from-blue-600 to-purple-600 text-white p-4 md:p-6 shrink-0">
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl md:text-2xl font-bold">Asistente Inteligente</h1>
                <p className="text-white/80 text-sm md:text-base">{getWelcomeMessage()}</p>
              </div>
            </div>
          ...
          {/* Métricas del sistema - Grid responsive */}
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 md:gap-4">
            {systemMetrics.map((metric, index) => {
              const IconComponent = metric.icon;
              return (
                <div key={index} className="bg-white/10 backdrop-blur-sm rounded-lg p-3 border border-white/20 min-w-0">
                  <div className="flex items-center gap-2">
                    <IconComponent className="h-4 w-4 md:h-5 md:w-5 text-white shrink-0" />
                    <div className="min-w-0 flex-1">
                      <p className="text-lg md:text-2xl font-bold text-white truncate">{metric.value}</p>
      ...
      <div className="border-b bg-white/50 backdrop-blur-sm sticky top-0 z-10 shrink-0">
              ...
              {availableTabs.map(tab => {
                ...
                return (
                  <TabsTrigger 
                    key={tab.id} 
                    value={tab.id} 
                    className="flex flex-col sm:flex-row items-center gap-1 sm:gap-2 py-3 sm:py-4 px-2 sm:px-6 data-[state=active]:bg-primary data-[state=active]:text-primary-foreground rounded-none border-b-2 border-transparent data-[state=active]:border-primary text-xs sm:text-sm"
                  >
      ...
      <div className="flex-1 overflow-hidden">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="h-full">
          <div className="h-full">
            ...
            <TabsContent value="overview" className="h-full overflow-auto space-y-4 md:space-y-6 mt-0 p-4 md:p-6">
              <div className="max-w-7xl mx-auto space-y-4 md:space-y-6">
                  ...
                  <CardContent>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                      {hasPermission('crear_reporte') && (
                        <Button 
                          variant="outline" 
                          className="h-16 md:h-20 flex flex-col gap-2 hover:bg-blue-50 hover:border-blue-200 text-xs md:text-sm"
                      ...
                      {hasPermission('ver_reporte') && (
                        <Button 
                          variant="outline" 
                          className="h-16 md:h-20 flex flex-col gap-2 hover:bg-green-50 hover:border-green-200 text-xs md:text-sm"
                      ...
                      {isAdmin() && (
                        <>
                          <Button 
                            variant="outline" 
                            className="h-16 md:h-20 flex flex-col gap-2 hover:bg-purple-50 hover:border-purple-200 text-xs md:text-sm"
                          ...
                          <Button 
                            variant="outline" 
                            className="h-16 md:h-20 flex flex-col gap-2 hover:bg-orange-50 hover:border-orange-200 text-xs md:text-sm"
                            onClick={() => window.open('/admin?tab=categories', '_blank')}
                          >
                            <FolderOpen className="h-5 w-5 md:h-6 md:w-6 text-orange-600" />
                            <span className="font-medium">Gestionar Categorías</span>
                          </Button>
                          
                          <Button 
                            variant="outline" 
                            className="h-16 md:h-20 flex flex-col gap-2 hover:bg-red-50 hover:border-red-200 text-xs md:text-sm"
                          ...
                          <Button 
                            variant="outline" 
                            className="h-16 md:h-20 flex flex-col gap-2 hover:bg-indigo-50 hover:border-indigo-200 text-xs md:text-sm"
                            onClick={() => window.open('/admin?tab=estados', '_blank')}
                          >
                            <Circle className="h-5 w-5 md:h-6 md:w-6 text-indigo-600" />
                ...
                {/* Resumen por áreas - Grid responsive */}
            ...
            <TabsContent value="analytics" className="h-full overflow-auto mt-0 p-4 md:p-6">
              <div className="max-w-7xl mx-auto">
                <AssistantDashboard onActionSuggestion={handleActionSuggestion} />
              </div>
            </TabsContent>

            <TabsContent value="reports" className="h-full overflow-auto mt-0 p-4 md:p-6">
              <div className="max-w-7xl mx-auto">
                <SmartReportsTracker />
              </div>
            </TabsContent>

            <TabsContent value="actions" className="h-full overflow-auto mt-0 p-4 md:p-6">
              <div className="max-w-7xl mx-auto">
            ...
            <TabsContent value="notifications" className="h-full overflow-auto mt-0 p-4 md:p-6">
              <div className="max-w-7xl mx-auto">
                <SmartNotifications />
              </div>
            </TabsContent>
