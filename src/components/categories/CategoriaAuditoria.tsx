...
export const CategoriaAuditoria: React.FC<CategoriaAuditoriaProps> = ({ categoriaId }) => {
  ...
  const { data: actividades = [], isLoading: isLoadingActividades } = useQuery({
    queryKey: ['categoria-actividades', categoriaId],
    queryFn: async () => {
      ...
      const actividadesFiltradas = (data as ActividadCategoria[]).filter(actividad => 
  ...
  const { data: cambios = [], isLoading: isLoadingCambios } = useQuery({
    queryKey: ['categoria-cambios', categoriaId],
    queryFn: async () => {
      ...
      return data as CambioCategoria[];
    }
  });
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          <span className="truncate">Auditoría de la Categoría</span>
        </CardTitle>
      </CardHeader>
      <CardContent>
          ...
          <TabsContent value="cambios" className="mt-4">
            <ScrollArea className="h-[300px]">
              {isLoadingCambios ? (
                ...
                <div className="text-center py-8 text-muted-foreground">
                  No se encontraron cambios registrados para esta categoría
                </div>
              ) : (
                <div className="overflow-x-auto">
                  <Table>
